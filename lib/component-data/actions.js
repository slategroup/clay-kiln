...
function clientSave(uri, data, oldData, store, {eventID, snapshot, paths}) { // eslint-disable-line
  console.log('in clientSave for ', uri)
  return modelSave(uri, data)
    .then((savedData) => {
      console.log('successfully ran modelSave for', uri)
      ...
      addToQueue(save, [uri, savedData, false], 'save') // add hooks=false to prevent models from re-running on server
        ...
        .catch((e) => {
          console.log('unable to add', uri, 'to the queue' , e)
    ...
    .catch((e) => {
      console.error('error in clientSave of ', uri, ': ', e)
...
export function saveComponent(store, {uri, data, eventID, snapshot, prevData}) {
  console.log('in saveComponent ', uri)
  ...
  const oldData = prevData || getData(uri),
    ...
    console.log('isPageSpecificComponent ', isPageSpecificComponent)
  ...
  return promise.then(() => {
    ...
    console.log('unable to save ', uri, e)
