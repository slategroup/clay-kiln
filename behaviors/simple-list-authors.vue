...
<script>
  ...
  import { UPDATE_FORMDATA, UPDATE_COMPONENT } from '../lib/forms/mutationTypes';
  ...
  export default {
    ...
    methods: {
      updateFormData() {
        console.log('hey in updateFormData ', this.items)
        console.log(this.name)

        const poorManSlugify = text => {
        ...
        let listAuthorRefs = this.items.map(name => {
          const uri = `local.dev.slate.com/components/author-bio/instances/${poorManSlugify(name.text)}`;
          this.$store.dispatch('getAuthor', uri);
          return {
            '_ref': uri
          }
        });
        this.$store.commit(UPDATE_FORMDATA, { path: this.name, data: this.items });
        this.$store.commit(UPDATE_FORMDATA, { path: 'authorBioContent', data: listAuthorRefs});
        console.log('stuff has maybe been committed chase ', listAuthorRefs)
      },
      onDoubleClick() {
    ...
    mounted() {
      ...
      // const uri = 'local.dev.slate.com/components/author-bio/instances/dan-kois';
      // getObject(uri).then((data) => {
      //   console.log('adding something fun! ', data)
      //   this.$store.commit(UPDATE_COMPONENT, { uri, data });
      //   console.log(data)
      // }).catch(e => {
      //   console.log('error!!! ', e)
      // });
